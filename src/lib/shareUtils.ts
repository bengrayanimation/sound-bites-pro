...
export async function generateQuoteCardImage(
  ...
  return new Promise((resolve) => {
    ...
    canvas.toBlob((blob) => resolve(blob!), mimeType, format === 'jpg' ? 0.95 : 1);
  });
}
...
export async function downloadQuoteAsJpg(quote: QuoteCard, style: string, filename: string, backgroundImageUrl?: string) {
  const blob = await generateQuoteCardImage(quote, style, backgroundImageUrl, 'jpg');
  downloadFile(filename, blob);
}
...
// Generate translated transcript HTML
export function generateTranslatedHtml(transcript: TranscriptSegment[], title: string, language: string): string {
  const segments = transcript.map(seg => `
    <div class="segment">
      <span class="timestamp">${formatTime(seg.startTime)}</span>
      ${seg.speaker ? `<span class="speaker">${seg.speaker}:</span>` : ''}
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${title} - ${language} Translation</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body {
      font-family: 'DM Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
    ...
    .speaker { font-weight: 600; color: #f4f4f5; margin-right: 0.5rem; }
    .text { color: #d4d4d8; line-height: 1.6; }
    .footer {
      text-align: center;
      margin-top: 2rem;
